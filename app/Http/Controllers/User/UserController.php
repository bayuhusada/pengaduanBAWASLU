...
class UserController extends Controller
{
    ...
    public function laporan($siapa = '')
    {
        ...
        if ($siapa == 'me') {
            $pengaduan = Pengaduan::where('nik', Auth::guard('masyarakat')->user()->nik)->leftJoin('tanggapan', 'tanggapan.id_pengaduan' ,'=', 'pengaduan.id_pengaduan')->leftJoin('petugas', 'petugas.id_petugas' ,'=', 'tanggapan.id_petugas')->orderBy('tgl_pengaduan', 'desc')->get(['pengaduan.*', 'tanggapan.*', 'petugas.nama_petugas']);

            return view('user.laporan', ['pengaduan' => $pengaduan, 'hitung' => $hitung, 'siapa' => $siapa]);
        } else {
            $pengaduan = Pengaduan::where([['nik', '!=', Auth::guard('masyarakat')->user()->nik], ['status', '!=', '0']])->leftJoin('tanggapan', 'tanggapan.id_pengaduan' ,'=', 'pengaduan.id_pengaduan')->orderBy('tgl_pengaduan', 'desc')->get(['pengaduan.*', 'tanggapan.*']);
